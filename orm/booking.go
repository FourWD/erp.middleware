...
type Booking struct {
	...
	// show user_type_id
	ReferralChannelID string `json:"referral_channel_id" query:"referral_channel_id" gorm:"type:varchar(2)"` //ช่องทางที่แนะนํา
	// ref sub for booking tar
	ReferralChannelSubID string     `json:"referral_channel_sub_id" query:"referral_channel_sub_id" gorm:"type:varchar(2)"` //ช่องทางย่อยที่แนะนํา
	ReferralName         string     `json:"referral_name" query:"referral_name" gorm:"type:varchar(256)"`                   // ชื่อผู้แนะนํา
	...
	Discount              float64    `json:"discount" query:"discount" gorm:"type:decimal(14,2)"`                      // ส่วนลด
	SpecialDiscount       float64    `json:"special_discount" query:"is_special_discount" gorm:"type:decimal(14,2)"`   // ส่วนลดพิเศษ
	SpecialDiscountBy     string     `json:"special_discount_by" query:"special_discount_by" gorm:"type:varchar(36)"`  // ส่วนลดพิเศษโดย
	SpecialDiscountDate   *time.Time `json:"special_discount_date" query:"special_discount_date" gorm:"type:date"`     // วันที่ส่วนลด
	SpecialDiscountRef    string     `json:"special_discount_ref" query:"special_discount_ref" gorm:"type:text"`       // หมายเหตุ
	SpecialDiscountRemark string     `json:"special_discount_remark" query:"special_discount_remark" gorm:"type:text"` // หมายเหตุ
